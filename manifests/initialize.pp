...
#  file {"${local_dir}/esb-agent.zip":
#    ensure => present,
#    source => 'puppet:///modules/esb/esb-agent.zip',
#  }
#
#  exec {'extract esb agent':
#    cwd     => $target,
#    command => "/usr/bin/unzip ${local_dir}/esb-agent.zip; /bin/chown -R ${owner}:${owner} ${target}/esb-agent",
#    unless  => "/usr/bin/test -d ${target}/esb-agent/",
#    require => File["${local_dir}/esb-agent.zip"];
#  }
